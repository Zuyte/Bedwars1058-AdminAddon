...
public class Messages {
    ...
    private final String ADDON_PATH = ".addons.admin.";

    public Messages() {
        if (!instance.isBedWarsProxy)
            setupBedWarsMessages();
        else
            setupProxyMessages();
    }

    private void setupBedWarsMessages() {
        for (Language lang : Language.getLanguages()) {
            YamlConfiguration yaml = lang.getYml();
            // Default Messages
    ...
    private void setupProxyMessages() {
        for (com.andrei1058.bedwars.proxy.api.Language lang : Admin.getInstance().bwProxy.getLanguageUtil().getLanguages()) {
            // Default Messages
