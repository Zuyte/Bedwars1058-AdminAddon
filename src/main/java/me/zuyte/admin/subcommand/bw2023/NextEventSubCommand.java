...
public class NextEventSubCommand {
    ...
    private void console(ConsoleCommandSender c, String[] args) {
        if (args.length > 1) {
            if (arenaUtil.getArenaByName(args[1]) == null) {
                TextUtils.sendDefaultConfigStringBW2023("defaults.arena-not-found", c);
                return;
            }
            IArena arena = arenaUtil.getArenaByName(args[1]);
            if (args.length >= 3) {
                ...
                if (found)
                    ...
                    TextUtils.sendDefaultConfigStringBW2023("admin-message.nextevent.error", c);
                return;
            }
        }
        TextUtils.sendDefaultConfigStringBW2023("usage.nextevent", c);
    }
